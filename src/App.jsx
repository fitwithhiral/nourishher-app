...
async function mailchimpSubscribe(email, name) {
  ...
  if (!apiKey || !audienceId || !server) {
    console.warn("⚠️ Mailchimp env vars missing — skipping email signup");
    return false;
  }

  try {
    // Generate MD5 of lowercase email for subscriber hash (required by Mailchimp)
    const emailLower = email.trim().toLowerCase();
    const md5 = await md5Hash(emailLower);

    // PUT request creates or updates subscriber (idempotent — safe to call multiple times)
    const url = `https://${server}.api.mailchimp.com/3.0/lists/${audienceId}/members/${md5}`;
...
// MD5 hash using SubtleCrypto (Mailchimp requires MD5 of lowercase email)
async function md5Hash(text) {
  // Mailchimp requires MD5; use a simple JS implementation since SubtleCrypto doesn't support MD5
  function md5(string) {
    ...
    function addUnsigned(x, y) {
      ...
      if (x4 | y4) { if (result & 0x40000000) return (result ^ 0xC0000000 ^ x8 ^ y8); else return (result ^ 0x40000000 ^ x8 ^ y8); }
      return (result ^ x8 ^ y8);
    }
    ...
    function convertToWordArray(str) {
      let lWordCount, lMessageLength = str.length, lNumberOfWordsTemp1 = lMessageLength + 8;
      let lNumberOfWordsTemp2 = (lNumberOfWordsTemp1 - (lNumberOfWordsTemp1 % 64)) / 64;
      let lNumberOfWords = (lNumberOfWordsTemp2 + 1) * 16, lWordArray = Array(lNumberOfWords - 1);
    ...
    function wordToHex(lValue) {
      ...
      for (lCount = 0; lCount <= 3; lCount++) { lByte = (lValue >>> (lCount * 8)) & 255; WordToHexValueTemp = "0" + lByte.toString(16); WordToHexValue = WordToHexValue + WordToHexValueTemp.substr(WordToHexValueTemp.length - 2, 2); }
      return WordToHexValue;
    }
    let x = convertToWordArray(string), k, AA, BB, CC, DD, a = 0x67452301, b = 0xEFCDAB89, c = 0x98BADCFE, d = 0x10325476;
